...
class Signup extends BaseController
{
	...
	{
		return view('Auth/Head').view('Auth/Register');
	}
	public function store()
	{
		...
		$validation->setRules(
			[
				...
				'inputDni' => 'required|regex_match[/^[0-9]{7,8}$/]',
				'inputFecNac' => 'required|chequeoEdad[' . $request->getPostGet('inputFecNac') . ']',
				'inputAddress' => 'required|alpha_numeric_space|min_length[3]',
				'inputTel' => 'required|regex_match[/^[0-9]{9,10}$/]',
			...
			[   // Errores-Mensajes
				...
				'inputDni' => [
					...
					'regex_match' => 'Debe ingresar un DNI valido',
				],
				'inputFecNac' => [
					'required' => 'Debe ingresar su fecha de nacimiento',
					'chequeoEdad' => 'Debe debe ser mayor de edad para registrarse',
				],
				'inputAddress' => [
				...
				'inputTel' => [
					...
					// 'alpha_numeric'=>'Debe ingresar solamente caracteres numericos',
				],
				'inputEmail' => [
		...
		if (!$validation->withRequest($this->request)->run()) {
			...
			if ($model->save($user)) {
				...
				return  redirect()->back()->with('msg', ['type' => 'danger', 'body' => 'Error al registrarse. Intentelo más tarde', 'typeInput' => 'is-invalid']);
			}
		}
	}
}
